...
function Task() {
      ...
      try {
        ...
        const userTasks =
          taskRes.data.data?.filter(
            (task) =>
              task.createdId === userId && task.assignedUserId !== userId
          ) || [];
        setTasks(userTasks);
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://task-management-1-al5b.onrender.com/task/${id}`
      );
  ...
  const renderTasks = () => {
    ...
    return (
      <ul>
        {tasks.map((task) => {
          const assignedUser = company.find(
            (user) => user._id === task.assignedUserId
          );
          return (
